...
- **Why (vs. alternatives):** Checked live DB: `fct_trials.is_terminated` is `TRUE` only for
  ...
  user before proceeding (schema had drifted from what the original requirements assumed). Derived label
...
- **What:** The original requirements listed 5 columns as "not in mart": `num_arms`, `masking`,
...
- **Why (vs. alternatives):** Fitting on the full dataset (including calib/test) is the common
  ...
  called out in the requirements, but directly on-theme for a leakage-focused portfolio project and cheap to
